...
function speak(text, rate = 0.8) {
  ...
  if (src) {
    try {
      ...
      audio.preload = "auto";
      audio.playbackRate = rate <= 0.75 ? Math.max(0.55, rate) : 1;
                ...
                return (
                  <div key={p.id} style={{ background: C.card, borderRadius: 20, padding: 20, marginTop: 14, border: `2px solid ${done ? C.jade : p.isWeak ? C.persimmon : C.line}` }}>
                    ...
                    <div style={{ display: "flex", gap: 8, marginTop: 14 }}>
                      ...
                      <button onPointerDown={(e) => speakSlowFromTap(e, p.en)} onClick={(e) => speakSlowFromTap(e, p.en)} onTouchEnd={(e) => speakSlowFromTap(e, p.en)} style={btn(C.goldSoft, C.gold, { flex: 1 })}>🐢 慢速</button>
